...
export function ItemsClient({ user }: { user: SessionUser }) {
  ...
  return (
    <div>
      ...
      {/* Card layout below sm — 7-8 columns don't fit a phone screen. */}
            ...
            <div key={item.id} className="rounded-lg border border-border p-3">
              <div className="flex items-start justify-between gap-2">
                <div className="min-w-0">
                  ...
                  <p className="text-sm text-muted-foreground">HSN {item.hsnCode} · {UNIT_LABELS[item.unit]}</p>
                </div>
                <Badge variant="secondary" className="shrink-0">{ITEM_CATEGORY_LABELS[item.category]}</Badge>
              </div>
              <div className="mt-2 flex items-end justify-between gap-2">
                <p className="text-sm text-muted-foreground">
                  Cost {inr.format(item.defaultCostPrice || 0)} · Sale {inr.format(item.defaultSalePrice || 0)}
                </p>
                <p className="font-heading text-base font-bold text-foreground">{item.gstRate}% GST</p>
              </div>
              {canEdit && (
